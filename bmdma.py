...
# Command discovery and dispatch moved to mdma_rebuild.commands.router during
# the V2 backend repo-prep. bmdma keeps a thin re-export here so that legacy
# references like ``bmdma.build_command_table`` and ``bmdma.advanced_cmds``
# continue to resolve for any external callers.
from mdma_rebuild.commands import router as _router
from mdma_rebuild.commands.router import build_command_table  # re-export

_router.load_command_modules()
_CMD_MODULES = _router._CMD_MODULES
AI_AVAILABLE = _router.AI_AVAILABLE
...
ai_cmds = _CMD_MODULES.get('ai_cmds')
